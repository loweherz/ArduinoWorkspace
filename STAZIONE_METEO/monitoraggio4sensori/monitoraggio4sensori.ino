...
#define Sound_PIN A0 //imposto il pin a cui è connesso il sensore
    ...
    if(time > SendTime + Periodo_Invio_Dati){
       ...
       if(n_camp > 0)
       {
         ...
         if (client.connect(serverName, serverPort)) //connessione al server per invio lettura sensore temperatura
         {
              ...
              InvioHttp(serverName,serverPort,pageName,username,password,id_Sound,avg_sound);
              client.stop();
